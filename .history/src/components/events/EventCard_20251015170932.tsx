    ...
    <div className="bg-white/50 backdrop-blur-lg rounded-xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-all duration-300 group">
      ...
      <div className="space-y-2 mb-4">
        ...
        <div className="flex items-center text-sm text-gray-500">
          ...
          {event.rsvpCount} RSVPs {event.capacity && `• ${event.capacity} max`}
        </div>
        {isFull && (
      ...
      <div className="flex space-x-2">
        <button 
          onClick={handleRsvp}
          disabled={isRsvping || (isFull && !hasUserRsvp)}
          className={`flex-1 py-2 px-4 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center ${
            hasUserRsvp
